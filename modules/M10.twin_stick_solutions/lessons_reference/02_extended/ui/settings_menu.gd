...
func _ready() -> void:
	_music_slider.value = db_to_linear(AudioServer.get_bus_volume_db(index_music_bus))
	print(_music_slider.value)
	_sounds_slider.value = db_to_linear(AudioServer.get_bus_volume_db(index_sounds_bus))
	...
	_sounds_slider.value_changed.connect(func (value: float) -> void:
